...
from gpcr_icl2.microswitches.structure import ResidueKey, ResidueRecord
from gpcr_icl2.receptor_only_dynamic_selectivity.features import (
    build_receptor_frame,
    contact_features,
    mechanical_features,
    normalise_generic,
)
from gpcr_icl2.receptor_only_dynamic_selectivity.modelling import grouped_prediction_permutation
...
def test_grouped_permutation_uses_receptor_blocks() -> None:
    rows = []
    ...
    for index in range(9):
        ...
        rows.append(
            {
                "sample_id": f"p{index}|r{index}|{label}",
                "pdb_id": f"p{index}",
                "receptor_name": f"r{index}",
                "transducer_family": label,
                "predicted_class": label,
                **{f"probability_{name}": probabilities[name] for name in labels},
            }
        )
    result = grouped_prediction_permutation(pd.DataFrame(rows), iterations=20, seed=7)
    assert result["status"] == "ok"
    assert result["iterations"] == 20
    assert 0 <= result["grouped_permutation_p_value"] <= 1
